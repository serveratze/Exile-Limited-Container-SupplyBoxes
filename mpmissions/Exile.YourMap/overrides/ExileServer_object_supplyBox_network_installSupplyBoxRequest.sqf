...
try
{
	...
	_supplyboxes = format ["getContainerSupplyBox:%1", (_territory getVariable ["ExileDatabaseID", 0]) ] call ExileServer_system_database_query_selectFull;
	_territoryLevelConfigs = getArray (missionConfigFile >> "CfgTerritories" >> "prices");
	_territoryLevelConfig = _territoryLevelConfigs select ((_territory getVariable ["ExileTerritoryLevel", 0]) - 1);
	_numberOfBoxesAllowed = _territoryLevelConfig select 4;
	if ( (count _supplyboxes) >= _numberOfBoxesAllowed ) then 
	{
		throw format["You can install Only %1 SupplyBoxes!",_numberOfBoxesAllowed];
	};
